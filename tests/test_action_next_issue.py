...
from actions.next_issue import NextIssue


class NextIssueActionTestCase(BaseActionTestCase):
    action_cls = NextIssue
    ...
    def setUp(self):
        self.maxDiff = None
        self.issues_graphql_response = json.loads(self.get_fixture_content("next_issue_api_response.json"))
        self.tdh_issue_graphql_response = json.loads(self.get_fixture_content("tdh_issue_api_response.json"))
        self.next_issue_plugin_response = json.loads(self.get_fixture_content("next_issue_plugin_response.json"))
        self.key_client = NextIssueActionTestCase.KeyMock()
    ...
    def test_next_issue(self, mock_get_key, mock_update_key, mock_api_url, mock_graphql):
        ...
        mock_graphql.side_effect = (self.issues_graphql_response, self.tdh_issue_graphql_response)
        # run action -> next_issue
        status, result = action.run()
        expected = self.next_issue_plugin_response
